...
export class Terminal {
  ...
  setText(text: string, col?: number, line?: number): void {
    ...
    let newPosition = this.iterateTiles(text.length, (tile, i) => {
      tile.char = text[i];
      dirtyTiles.push(tile);
    }, col, line);

    newPosition = this.iterateTiles(1, () => {}, newPosition.col, newPosition.line);
    this.setCursor(newPosition.col, newPosition.line);
  ...
  setTiles(tiles: Tile|Tile[], col?: number, line?: number): void {
    ...
    this.iterateTiles((<Tile[]>tiles).length, (tile, i) => {
   ...
   * @param col starting tile column
   * @param line starting tile line
   */
